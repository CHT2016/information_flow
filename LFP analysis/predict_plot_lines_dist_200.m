...
for ii=1:size(ncho,1)
    ...
    if band==1
        ...
        ttl=strrep([hemis, ': 2.5-12.5 HZ (X) to '],'_','\_');
    elseif band==2
        mtwhat(ii,:,:)=squeeze(mean(twhat(:,4:5,:),2));
        mtwhere(ii,:,:)=squeeze(mean(twhere(:,4:5,:),2));
        ttl=strrep([hemis, ': 12.5-22.5 HZ (X) to '],'_','\_');
    elseif band==3
        mtwhat(ii,:,:)=squeeze(mean(twhat(:,6:9,:),2));
        mtwhere(ii,:,:)=squeeze(mean(twhere(:,6:9,:),2));
        ttl=strrep([hemis, ': 22.5-42.5 HZ (X) to '],'_','\_');
        %     elseif band==4
        %         mtwhat(ii,:,:)=squeeze(mean(twhat(:,12:18,:),2));
        %         mtwhere(ii,:,:)=squeeze(mean(twhere(:,12:18,:),2));
        %         ttl=strrep([hemis, ': 41-68 HZ (X) to '],'_','\_');
    elseif band==4
        mtwhat(ii,:,:)=squeeze(mean(twhat(:,10:21,:),2));
        mtwhere(ii,:,:)=squeeze(mean(twhere(:,10:21,:),2));
        ttl=strrep([hemis, ': 42.5-100 HZ (X) to '],'_','\_');
    end
...
for j=[-3:-1,1:3]
    try
        ...
        peak_what(ij,:,2)=mean(tmat_what(4:5,:),1);
        peak_what(ij,:,3)=mean(tmat_what(6:9,:),1);
        %         peak_what(ij,:,4)=mean(tmat_what(12:18,:),1);
        peak_what(ij,:,4)=mean(tmat_what(10:21,:),1);
        ij=ij+1;
    end
end
xlabel('Frequency (Hz)')
...
for j=[-3:-1,1:3]
    try
        ...
        peak_where(ij,:,2)=mean(tmat_where(4:5,:),1);
        peak_where(ij,:,3)=mean(tmat_where(6:9,:),1);
        %         peak_where(ij,:,4)=mean(tmat_where(12:18,:),1);
        peak_where(ij,:,4)=mean(tmat_where(10:21,:),1);
        ij=ij+1;
    end
end
xlabel('Frequency (Hz)')
...
tttl={'2.5-12.5 HZ', '12.5-22.5 HZ', '22.5-42.5 HZ', '42.5-100 HZ'};
for mm=band:4
    subplot(2,3,2+mm)
    tData=peak_what(:,:,mm)';

...
bins=3:11; % new period is too colse to the default one.
bincen=Bin.cen;
...
%                 data1=10*log10(P1{1, tarray1(m)}(:,:,iWW)-P1{1, tarray1(m)}(:,1,iWW)); % minuse baseline
                for n=1:length(tarray2)
                    data2=10*log10(P1{1, tarray2(n)}(:,:,iWW));
%                     data2=10*log10(P1{1, tarray2(n)}(:,:,iWW)-P1{1, tarray2(n)}(:,1,iWW)); % minuse baseline
                    for iii=1:length(bins)
                        i=bins(iii);
%                         X=squeeze(data1(:,i,:))'; 
%                         Y=squeeze(data2(:,i,:))';
                        X=squeeze(data1(:,i,:)-data1(:,1,:))'; % minuse baseline
                        Y=squeeze(data2(:,i,:)-data2(:,1,:))';
...
%                         Mebr=Br*Ar';
        ...
        save(['lfp_predict_', fname, '_' hemi{ihemi} '_' whatwhere{iww},'_mb'],'Maa','Maar','Bin','tarray','ncho','nf')
    end
end
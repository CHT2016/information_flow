function [decoa,deco_mat]=ww_decoding_dir_byblock(cSpike,indM,bin_cen,stimdir,gBlock,varargin)
...
for g=1:length(bin_cen)
    ...
    for m=1:numB % in this codition, objects in each block were different.
        ...
        tindM=indM(row);
        [C,~,~]=unique(stimdir(indM(row),:));
        indC=[];Matrix=[];
        
        if numel(C)==2
            indC{1}=intersect(find(stimdir==C(1)),tindM);
            indC{2}=intersect(find(stimdir==C(2)),tindM);
            ...
            for n=1:2
                ...
                for o=1:length(ind)
                    ...
                    for p = 1:size(tMatrix,2)% stimdir(triind,:)% in what & where task, only has two chioce options

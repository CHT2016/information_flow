...
for kk=1:length(fnames)
    ...
    for k = 1: size(Neurons,1)
        ...
        for ib=1:length(ubindx)
            ...
            if numel(find(allTS0>0 & allTS0<500))>= numel(find(allTS1>0 & allTS1<500)) %%aligned to cue
...
[ivArray,nindex]=NS_nAnova_sign(indBin,Bin,iArray,nc);
ivArray{1,2}='dir';
ivArray{2,2}='obj';
...
for i=1:2
    for j=1:4
        ...
        Data{1}=squeeze(psth_what(1,:,:))';
        Data{2}=squeeze(psth_what(2,:,:))';
        Data{3}=squeeze(psth_where(1,:,:))';
        Data{4}=squeeze(psth_where(2,:,:))';
...
%     load('rlPlaceImg-voltaire-10-17-2016-BHVeventsNIP1.mat')
    ...
    for ib=1:length(ubindx)
        ...
        allTS0=[];allTS1=[];
        for j=1:length(whatindx0)
            TS = tts{1, whatindx0(j)}.choiceSpikes; %%aligned to cue
            allTS0 = [allTS0,TS];
        end
        for j=1:length(whatindx1)
            TS = tts{1, whatindx1(j)}.choiceSpikes; %%aligned to cue
            allTS1 = [allTS1,TS];
        end

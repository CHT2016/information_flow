function [aveIntp, aveBasep, avePrecue, onset]=ANOVA_ave_latency_plot(Data,para)
...
intbin=[201:351] % 101:176 for decoding; 201:351 for anova
...
for m=1:length(Data)
    ...
    if isfield(para,'smooth') & para.smooth=='Gaussian'
        [gs_ave_fResult, ~] = smooth1d(ave_fResult', (1:length(ave_fResult))', 10, 1);
        [gs_SEM_fResult, ~] = smooth1d(SEM_fResult', (1:length(SEM_fResult))', 10, 1);
    ...
    try
        tonset=bins(ind1);
        col = find(tonset> -0.5 & tonset< 1.5);
    ...
    try
        text(0+m*0.2, para.ylim(2)-0.1-m*0.01,num2str(bins(ind1)'),'color',para.colors{m})
    end
end
